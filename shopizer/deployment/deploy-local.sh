...
#   --admin-tag   <tag>    Image tag for admin UI  (e.g. 1.0.0-a1b2c3d4)
#   --react-tag   <tag>    Image tag for React     (e.g. 2.0.0-a1b2c3d4)
...
[[ -n "$ADMIN_TAG"   ]] && IMAGE_TAG_ADMIN="$ADMIN_TAG"
[[ -n "$REACT_TAG"   ]] && IMAGE_TAG_REACT="$REACT_TAG"
...
if [[ "$ONLY_BACKEND" == "true" ]]; then
  ...
  pull_and_load "backend" "$IMAGE_TAG_BACKEND" "docker-image-backend-*"
  pull_and_load "admin"   "$IMAGE_TAG_ADMIN"   "docker-image-admin-*"
  pull_and_load "react"   "$IMAGE_TAG_REACT"   "docker-image-react-*"
fi
...
update_env "IMAGE_TAG_ADMIN"   "$IMAGE_TAG_ADMIN"
update_env "IMAGE_TAG_REACT"   "$IMAGE_TAG_REACT"

echo ""
info "Active image tags:"
echo -e "  Backend : ${BOLD}${IMAGE_TAG_BACKEND}${RESET}"
echo -e "  Admin   : ${BOLD}${IMAGE_TAG_ADMIN}${RESET}"
echo -e "  React   : ${BOLD}${IMAGE_TAG_REACT}${RESET}"
echo ""
...
echo -e " Admin UI    : http://localhost:${ADMIN_PORT:-4200}"
